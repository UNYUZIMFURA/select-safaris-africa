...
const Partners = () => {
    const arrayImgs = [Company1, Company2, Company3, Company4, Company5, Company6, Company7, Company8]
    return (
      <div className="flex flex-col bg-white items-center py-8">
        <div className="flex flex-col items-center gap-6 text-center max-w-[30rem] lg:max-w-[35rem]">
          ...
          <h2 className="font-bold text-2xl text-tertiary2 md:text-[1.6rem] lg:text-[1.9rem] leading-10 xl:text-[2.2rem] 2xl:text-[2.5rem] xl:leading-[3.3rem]">
            Checkout the companies that <span className="text-brand">we partner with</span>
          </h2>
        ...
        <div className="w-full grid grid-cols-2 justify-items-center gap-8 md:grid-cols-3 md:justify-between py-10 2xl:px-[8rem]">
           {
            arrayImgs.map((img,index) => (
                <Image key={index} src={img} height={180} width={180} alt="" className="cursor-pointer"/>
            ))
           }
        </div>
      </div>
